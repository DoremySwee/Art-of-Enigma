...
events.onWorldTick(function(event as crafttweaker.event.WorldTickEvent){
    ...
    for heart in world.getEntities(){
        ...
        if(heart.nbt has "ForgeData" && heart.nbt.ForgeData has "dragonHeartCraftData"){
            ...
            heart.posY=data.y.asDouble()+(1.0-L.cosf(0.0+Math.min(180,data.t.asInt())))*0.3;
            ...
            if(stage==0){
                ...
                for i in L.getEntitiesAround(world,V.getPos(heart),10){
                    if(i instanceof IEntityItem){
                ...
                if(timer>130){
                    ...
                    for i in 0 to 4{
                        if(data.bpdata has "bp"~i){
                            ...
                            if(L.isBlock(world, pos, "draconicevolution:draconium_block",1)){
                                //L.executeCommand("setblock draconicevolution:draconic_block "~pos.x~" "~pos.y~" "~pos.z);
                                //L.say("setblock draconicevolution:draconic_block "~pos.x~" "~pos.y~" "~pos.z);
                    ...
                    for r in 0 to 5{
                        ...
                        for dpos in t{
                            if(bp.length>=count/4)break;
                            var pos = L.formBlockPos(heart.x+dpos[0],heart.y+dpos[1],heart.z+dpos[2]);
                            if(L.isBlock(world,pos,"draconicevolution:draconium_block",1)){
                    ...
                    for i in 0 to 4{
                        if(data.bpdata has "bp"~i){
                            ...
                            if(L.isBlock(world, IBlockPos.create(pos[0],pos[1],pos[2]),"draconicevolution:draconium_block",1)){
                                //L.say("c");
                                bp+=pos;
